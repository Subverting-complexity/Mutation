...
| Keep the pointer put for (milliseconds) | 1500 by default, and anything up to 10000. After a capture, Mutation watches the mouse pointer for this long and puts it back if something else moves it. Some magnifiers move the pointer to the flashing text cursor of whatever app comes back to the front. Touching the mouse yourself stops the watch at once, so it never fights you. Set it to 0 to switch it off. |
