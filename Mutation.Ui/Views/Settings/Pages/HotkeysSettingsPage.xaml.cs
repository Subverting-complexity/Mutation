...
public sealed partial class HotkeysSettingsPage : UserControl
{
	...
	private static readonly HotkeySpec[] Specs = new[]
	{
		...
		new HotkeySpec("Speech to text + format with LLM",
			s => s.SpeechToTextSettings?.SpeechToTextWithLlmFormattingHotKey,
			(s, v) => (s.SpeechToTextSettings ??= new SpeechToTextSettings()).SpeechToTextWithLlmFormattingHotKey = v,
			false, SettingsDefaults.Speech.SpeechToTextWithLlmFormattingHotKey),
		...
		new HotkeySpec("Format with LLM",
			s => s.LlmSettings?.FormatWithLlmHotKey,
			(s, v) => (s.LlmSettings ??= new LlmSettings()).FormatWithLlmHotKey = v,
			true, SettingsDefaults.Llm.FormatPromptHotKey),

		new HotkeySpec("Text to speech",
			s => s.TextToSpeechSettings?.TextToSpeechHotKey,
			(s, v) => (s.TextToSpeechSettings ??= new TextToSpeechSettings()).TextToSpeechHotKey = v,
			false, SettingsDefaults.Tts.TextToSpeechHotKey),
